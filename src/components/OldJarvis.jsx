    ...
    try {
      ...
      if (e.name === 'QuotaExceededError') {
        ...
        try {
          ...
          console.error('Failed to save to localStorage:', innerError);
        }
      }
    }
  },
...
const AIChat = ({ isExpanded = false }) => {
  ...
  const initializeChat = async (apiKey) => {
    try {
      ...
      // Initialize chat with identity and history
      const chat = model.startChat({
        history: [
          {
            role: "model",
            parts: [{ text: JARVIS_IDENTITY }]
          },
          ...chatHistory // Include all previous conversation history
        ],
        generationConfig: {
...
const sendMessage = async () => {
  ...
  try {
    ...
    let messageParts = inputValue;
    if (selectedImage) {
      const imagePart = await fileToGenerativePart(selectedImage);
      messageParts = [imagePart, { text: inputValue }];
    }

    try {
      const result = await rateLimitedSendMessage(chatInstance, messageParts);
      const response = result.response.text();
      ...
      // Try to handle API quota error
      const retryResult = await handleApiError(error, messageParts);
      if (retryResult) {
        const response = retryResult.response.text();
        setMessages(prev => prev.filter(msg => !msg.isLoading).concat({
          sender: 'Jarvis',
          text: response
        }));
    ...
    setMessages(prev => prev.filter(msg => !msg.isLoading).concat({
      sender: 'Jarvis',
      text: 'I apologize, but all available API keys are currently exhausted. Please try again later.'
    }));
  } finally {
    setIsLoading(false);
